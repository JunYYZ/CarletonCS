...
bool FileList::add(File *newFile)
{
    ...
    // Insert at head if newFile is less than head
    else if (newFile->lessThan(*head->file))
    {
        newNode->next = head;
        head->prev = newNode;
        head = newNode;
    ...
    {
        FileNode *current = head;
        while (current->next && !newFile->lessThan(*current->next->file))
        {
            current = current->next;
        }
