...
public class DecodeB {
	...
	public static int decodeB(InputGenerator<Integer> gen, int k) {
		...
		while (gen.hasNext()) {
			...
			if (gen.hasNext()) {
				...
				window.add(current);
				int currFreq = freqMap.getOrDefault(current, 0) + 1;
				freqMap.put(current, currFreq);

				freqCount.put(currFreq, freqCount.getOrDefault(currFreq, 0) + 1);
				if (currFreq > 1) {
					freqminus(freqCount, currFreq - 1);
				}

				if (window.size() > k) {
					Integer exiting = window.poll();
					int freq = freqMap.get(exiting);
					freqMap.put(exiting, freq - 1);
					freqminus(freqCount, freq);
					if (freq - 1 > 0) {
	...
	// helper method to decrement the frequency count in TreeMap
	private static void freqminus(TreeMap<Integer, Integer> freqCount, int freq) {
	...
	public static void main(String[] args) {
		...
		gen = new FileIntGenerator();
		System.out
				.println("Enter a sequence of positive integers, then input -1 and hit Enter to run decodeB(gen, 3):");
