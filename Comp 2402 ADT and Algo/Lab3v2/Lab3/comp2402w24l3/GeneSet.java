...
//	}
	...
	private void promoteNode0(Node target) {
		if (target == null) {
	...
	private void promoteNode(Node target) {
		// "optimized" version
		if (target == null || target.height() >= 32) {
			return;
		}
		...
		Node prev = dummy;
		while (prev.next[newLevel] != null && prev.next[newLevel].s.compareTo(target.s) < 0) {
			prev = prev.next[newLevel];
		}

		target.next[newLevel] = prev.next[newLevel];
		prev.next[newLevel] = target;
	}
...
//	}
	...
	public String find(String s) throws IllegalArgumentException {
		...
		while (r >= 0) {
			int counter = 0;
			while (current.next[r] != null && current.next[r].s.compareTo(s) < 0) {
...
//					}
			...
			if (current.next[r] != null && current.next[r].s.compareTo(s) >= 0) {
				if (current.next[r].s.compareTo(s) == 0) {
					counter++;
				}
				if (counter >= 2) {
//					if (current != dummy) {
...
//					}
	...
	public boolean remove(String s) throws IllegalArgumentException {
		...
		while (r >= 0) {
			...
			// element, though this one is almost certainly redundant
			if (current.next[r] != null && current.next[r].s.compareTo(s) >= 0) {
				if (counter >= 2) {
					if (current != dummy) {
						promoteNode(current.next[r]);
...
//			for (int i = 0; i <= h && preds[i].next[i] == current; i++) { // was to catch the randoms
