...
public class JumbleB extends Jumble {
	...
	public String jumble(InputGenerator<Character> gen) {
		// exact same logic as jumbleA
		DLList<Character> dlList = new DLList<>();
		StringBuilder sb = new StringBuilder(dlList.size());
		ListIterator<Character> iterator = dlList.listIterator();
		...
		while (gen.hasNext()) {
			...
			if (c2 == 'A' && c1 == 'U') {
				if (current == 'C') {
					int stepsBack = cursor / 2;
					while (stepsBack-- > 0 && iterator.hasPrevious()) {
						iterator.previous();
					}
				} else if (current == 'G') {
					int stepsForward = cursor / 4;
					while (stepsForward-- > 0 && iterator.hasNext()) {
						iterator.next();
					}
				}
			}
...
//		// of
//		// characters of your own devising and then run jumbleB once you hit 'Enter',
//		// then
